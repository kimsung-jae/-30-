...
public class MainActivity extends Activity{
    ...
    private View buildUi(){
        ...
        root.addView(tv("보글사다리3 · V7.2 Binary480 Rolling",24,Color.WHITE,true));
        TextView sub=tv("API 스냅샷 누적 · 최근 최대480회 · 이진 3차원 · 최근3~6 완전패턴",12,Color.rgb(110,231,183),false);
        ...
        refresh=button("🔄 지금 추첨 / Binary480 분석",Color.rgb(5,150,105));
        ...
        hero.addView(tv("다음 회차 V7.2 Binary480 삼치기",12,Color.GRAY,false));
        ...
        hero.addView(grade);
        TextView note=tv("● API 스냅샷을 idx로 합쳐 최근 최대480회 유지",12,Color.rgb(110,231,183),true);
        note.setPadding(0,dp(10),0,0);
        hero.addView(note);
        root.addView(hero);

        LinearLayout fc=card();
        fc.addView(section("좌우 / 사다리수 / 홀짝 · Rolling480 이진분석"));
        ...
        bc.addView(section("최근 최대480회 · 미래누설 없는 순차 재현"));
        backtest=tv("-",14,Color.WHITE,false);
        backtest.setLineSpacing(0,1.3f);
        bc.addView(backtest);
        live=tv("V7.2 실전 기록 · 아직 없음",14,Color.rgb(125,211,252),true);
        ...
        backup=button("💾 V7.2 백업",Color.rgb(21,128,61));
        ...
        dc.addView(dr);
        reset=button("V7.2 성적만 초기화",Color.rgb(127,29,29));
        LinearLayout.LayoutParams rlp=new LinearLayout.LayoutParams(-1,dp(46));
        rlp.setMargins(0,dp(8),0,0);
        dc.addView(reset,rlp);
        root.addView(dc);

        root.addView(tv("※ V7.2는 API가 한 번에 주는 스냅샷을 중복 제거해 누적하고, 그중 최신 최대480회만 이진 분해해 분석합니다. 무작위라면 삼치기 기준 성공률은 75%입니다.",11,Color.GRAY,false));
        return sv;
    }
    ...
    private void manualSync(){
        ...
        status.setText("API 스냅샷 수집 + Binary480 분석 중...");
        ex.execute(()->{
            try{
    ...
    private void render(FlowCore.Analysis a,List<FlowCore.Result>d){
        ...
        f.append("Rolling 분석 데이터: ").append(a.count480).append("/480회")
                ...
                .append("\n\n가중치 없음 · 이진3차원 · 최근3~6 완전패턴")
                .append("\nAPI 스냅샷은 중복 제거해 최근 최대480회까지만 누적");
        ...
        backtest.setText(
                "Rolling480 내부 순차재현: "+stat(b.hit,b.n)+
                "\n\n[3차원 최종방향 적중]"+
                ...
                "\n\n※ 이 재현값은 현재 보유 Rolling480 안의 참고값 · 실전 누적과 별도"+
                "\n기준 삼치기: 75.0%"
        );
        ...
        live.setText("V7.2 실전 · "+(n>0?hit+"/"+n+" = "+FlowCore.pct((double)hit/n)+" · 누적 "+FlowCore.signed(lp):"아직 없음"));
        ...
        profit.setText(
                ...
                "\nRolling480 내부 순차재현 가상손익: "+FlowCore.signed(bt)
        );
    ...
    private void startExport(){
        ...
        i.putExtra(Intent.EXTRA_TITLE,"BubbleBinary480V72_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.KOREA).format(new Date())+".json");
    ...
    @Override protected void onActivityResult(int req,int res,Intent data){
        ...
        try{
            if(req==REQ_EXPORT){
                ...
                o.close();
                Toast.makeText(this,"V7.2 백업 완료",Toast.LENGTH_LONG).show();
                ...
                loadSettings();
                reloadAsync();
                Toast.makeText(this,"가져오기 완료 · 이후 API 스냅샷과 합쳐 최근480회 유지",Toast.LENGTH_LONG).show();
            }
        }catch(Exception e){
    ...
    private void confirmReset(){
        new AlertDialog.Builder(this)
                .setTitle("V7.2 실전성적 초기화")
                .setMessage("실전 승패·수익만 초기화합니다. 현재 Rolling480 분석 데이터는 그대로 보존됩니다.")
