...
public final class FlowCore {
    ...
    public static final String PREF="bubble_binary30_v7";
    public static final String ACTION_UPDATED="com.bubbleladder.binary30v7.FLOW_UPDATED";

    public static final String K_HISTORY="api30_snapshot_v7", K_RECORDS="records_v7",
            K_PENDING_IDX="pending_idx_v7", K_PENDING_EXCLUDE="pending_exclude_v7",
            K_PENDING_STAKE="pending_stake_v7", K_PENDING_ODDS="pending_odds_v7",
            K_PENDING_GRADE="pending_grade_v7",
            K_LIVE_TOTAL="live_total_v7", K_LIVE_SUCCESS="live_success_v7",
            K_LIVE_PROFIT="live_profit_v7", K_BASE_STAKE="base_stake_v7",
            K_ODDS="odds_v7", K_AUTO="auto_enabled_v7",
            K_LAST_EXCLUDE="last_exclude_v7", K_LAST_TRIPLE="last_triple_v7",
            K_LAST_GRADE="last_grade_v7", K_LAST_MODE="last_mode_v7",
            K_LAST_SYNC="last_sync_v7", K_LAST_API_COUNT="last_api_count_v7";
    ...
    public static final class Analysis{
        public int exclude,count30,topOppose,secondOppose,decisiveVotes;
    ...
    public static List<Result> fetch() throws Exception{
        ...
        c.setRequestProperty("User-Agent","BubbleBinary30/7.0");
    ...
    // 핵심: 이전 history와 절대 merge하지 않는다. 매번 API 최신 30회로 교체한다.
    public static SyncResult sync(Context c)throws Exception{
        ...
        List<Result> snapshot=fetch();
        boolean resolved=resolvePending(c,snapshot);
        save(c,snapshot);
        Analysis a=analyze(snapshot);
        savePending(c,snapshot,a);
        prefs(c).edit().putLong(K_LAST_SYNC,System.currentTimeMillis()).apply();

        SyncResult sr=new SyncResult();
        sr.newRoundResolved=resolved||(!snapshot.isEmpty()&&snapshot.get(0).idx!=latestBefore);
        sr.analysis=a;sr.history=snapshot;
        return sr;
    }
    ...
    public static Analysis analyze(List<Result> desc){
        ...
        a.exclude=d.exclude;a.count30=end;a.date=dayKey(all.get(end-1).date);
    ...
    private static FlowDecision flowDecision(List<Result> all,int start,int end){
        ...
        if(leaders.size()==1){
            d.exclude=leaders.get(0);d.mode="API30 이진 최근3~6 · 반대표 최다";
        }else{
            ...
            else{d.exclude=tieByAbsence(all,start,end,second);d.mode="동률 → API30 미출현 간격";}
        }
    ...
    public static JSONObject backup(Context c)throws Exception{
        SharedPreferences sp=prefs(c);JSONObject root=new JSONObject();root.put("format","BubbleBinary30V7Backup");
        root.put("snapshot",new JSONArray(sp.getString(K_HISTORY,"[]")));root.put("records",new JSONArray(sp.getString(K_RECORDS,"[]")));
    ...
    public static void restore(Context c,JSONObject root)throws Exception{
        SharedPreferences.Editor ed=prefs(c).edit();JSONObject st=root.optJSONObject("state");
        if(root.has("snapshot")){
            JSONArray src=root.getJSONArray("snapshot"),cut=new JSONArray();
            for(int i=0;i<Math.min(WINDOW,src.length());i++)cut.put(src.get(i));
